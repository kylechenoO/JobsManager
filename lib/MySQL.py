...
class MySQL(object):
    ...
    def __init__(self, logger: object, host: str, port: str, username: str, password: str, database: str, charset: str) -> None:
        """
    ...
    def connect(self) -> bool:
        ...
        ## debug prt
        self.logger.debug({'db_host': self.db_host})
        self.logger.debug({'db_port': self.db_port})
        self.logger.debug({'db_username': self.db_username})
        self.logger.debug({'db_password': self.db_password})
        self.logger.debug({'db_database': self.db_database})
        self.logger.debug({'db_charset': self.db_charset})
        ...
        try:
            ...
            if self.con.open:
                ...
                self.logger.info({'status': 'successfully connected to MySQL database %s at %s:%s' % (self.db_database, self.db_host, self.db_port)})
                self.logger.debug({'status': 'end'})
                return True
        ...
        ## error handling
            ...
            self.logger.error({'status': "Error connecting to MySQL: %s" % str(e)})
            self.con = None
            self.cur = None
            self.logger.debug({'status': 'end'})
            return False
